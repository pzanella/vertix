...
import { Canvas, Controls, Timeline } from "./components/Player";
...
export default function App() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center gap-10 p-6">
      <header className="flex flex-col items-center gap-1 pt-8">
      ...
      {state === "idle" && (
        ...
        >
          ...
          <span className="text-neutral-400 text-sm">
            Drop a 16:9 video here, or click to choose one
          </span>
          {!wasmReady && (
            <span className="text-neutral-600 text-xs">Warming up the reframe engine…</span>
          )}
          <input
            ref={fileInputRef}
            type="file"
      ...
      {state === "error" && (
        <div className="flex flex-col items-center gap-3 max-w-lg text-center">
          <p className="text-neutral-300 text-sm">
            That file didn't load. Try a different video.
          </p>
          <button
            onClick={() => fileInputRef.current?.click()}
