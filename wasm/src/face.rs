...
impl FaceTracker {
    ...
    /// Runs detection on a 320x240 letterboxed RGBA frame. Returns the
    /// chosen face's center-X in the same 0..320 pixel space the saliency
    /// fallback uses, or `None` if nothing trustworthy was found.
    pub fn update(&mut self, rgba: &[u8]) -> Option<f64> {
        ...
        for c in &candidates {
            ...
            let hysteresis = match self.last_center_x {
                Some(last) => 1.0 - ((cx - last).abs() as f64).min(1.0),
                None => 0.0,
            };
            ...
            let combined = motion + hysteresis * 5.0 + c.score as f64 * 2.0;

            if best.is_none_or(|(s, _)| combined > s) {
                best = Some((combined, cx));
            }
        }
